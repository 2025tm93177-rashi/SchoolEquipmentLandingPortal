  ...
  db.serialize(() => {
    ...
    `,
      (err) => {
        if (err) {
          console.error(
            "Error creating equipment_requests table:",
            err.message,
          );
        } else {
    ...
    db.run(
      `CREATE INDEX IF NOT EXISTS idx_requests_user_id ON equipment_requests(user_id)`,
    );
    db.run(
      `CREATE INDEX IF NOT EXISTS idx_requests_equipment_id ON equipment_requests(equipment_id)`,
    );
    db.run(
      `CREATE INDEX IF NOT EXISTS idx_requests_status ON equipment_requests(status)`,
    );
    db.run(
      `CREATE INDEX IF NOT EXISTS idx_requests_date ON equipment_requests(request_date)`,
    );
...
const seedAdminUsers = () => {
  db.get("SELECT COUNT(*) as count FROM users", [], (err, row) => {
    ...
    if (row.count === 0) {
        ...
        db.run(
          ...
          function (err) {
            if (err) console.error("Error creating admin1:", err.message);
              ...
              console.log("Created admin1@school.edu (ID:", this.lastID, ")");
          },
        );
        ...
        db.run(
          ...
          function (err) {
            if (err) console.error("Error creating admin2:", err.message);
              ...
              console.log("Created admin2@school.edu (ID:", this.lastID, ")");
          },
        );
        ...
        db.run(
          ...
          function (err) {
            if (err) console.error("Error creating student:", err.message);
              ...
              console.log("Created student@school.edu (ID:", this.lastID, ")");
          },
        );
        ...
        db.run(
          ...
          function (err) {
            if (err) console.error("Error creating teacher:", err.message);
              ...
              console.log("Created teacher@school.edu (ID:", this.lastID, ")");
              ...
              db.get(
                ...
                (err, user) => {
                  if (err) {
                    ...
                    console.log(
                      "\nVerification: admin1 password_hash is",
                      user.password_hash ? "SET" : "NULL",
                    );
                    console.log(
                    ...
                    console.error(
                      " Verification failed: admin1 user not found",
                    );
                  }
                },
              );
      ...
      console.log(
        " To re-seed, delete database and restart, or run: node scripts/force-seed.js\n",
      );
    }
  });
};
...
const seedEquipment = () => {
  db.get("SELECT COUNT(*) as count FROM equipment", [], (err, row) => {
    ...
    if (row.count === 0) {
      ...
      console.log(
        `Equipment table already has ${row.count} items - skipping seed\n`,
      );
    }
  });
};
